...
function checkProp(property, condition) {
    ...
    switch(symbol) {
        ...
        case '!=':
            ...
            return propData != conditionData;
        case '?': {
            // conditionData may be a scalar (e.g. `EVT?900305`) or an array
            // (e.g. `AGE?[1,2,3]`); normalize so membership checks always work.
            const set = Array.isArray(conditionData) ? conditionData : [conditionData];
            if(Array.isArray(propData)) {
                for(const p of propData)
                    if(set.includes(p)) return true;
                return false;
            }
            return set.includes(propData);
        }
        case '!': {
            const set = Array.isArray(conditionData) ? conditionData : [conditionData];
            if(Array.isArray(propData)) {
                for(const p of propData)
                    if(set.includes(p)) return false;
                return true;
            }
            return !set.includes(propData);
        }
